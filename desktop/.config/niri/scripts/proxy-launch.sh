...
# Parse [Desktop Entry] only. Output: name TAB exec TAB terminal
...
if [[ "$exec_cmd" == *"flatpak run"* ]]; then
    eval "${exec_cmd/flatpak run/flatpak run --env=HTTP_PROXY=$PROXY_HTTP --env=HTTPS_PROXY=$PROXY_HTTP --env=http_proxy=$PROXY_HTTP --env=https_proxy=$PROXY_HTTP --env=ALL_PROXY=$PROXY_SOCKS}" &>/dev/null &
